...
function ShapeSWAXS(v::Voxel, d::AFloat, q::AVec; J::Int64=1200, sd::Float64=0.335)

    mat = v.coordata;
    if v.dim == 51
        dx = 2.0;
    else
        dx = norm(v.coordata[1, :] .- v.coordata[2, :]);
    end

    dd = (d * dx ^ 3) * ones(size(mat, 1));
    solventden = sd * dx ^ 3;

    intensity = pmap(x -> _DQ(mat, dd, x; sd=solventden, J=J), q, distributed=true);
    return intensity;
end
